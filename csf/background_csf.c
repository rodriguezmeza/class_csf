...
int background_functions(
    ...
    rho_m += pvecback[pba->index_bg_rho_b];
        ...
        pvecback[pba->index_bg_rho_b] = pba->Omega0_b * pow(pba->H0,2) / pow(a,3)
            * exp(-theta_SCF(pba, a)) // here define CSF(tau)
        ;
        rho_tot += pvecback[pba->index_bg_rho_b];
        p_tot += 0;
        rho_m += pvecback[pba->index_bg_rho_b];
    ...
    if (pba->has_CSF == _FALSE_) {
        ...
        if (pba->has_lambda == _TRUE_) {
            ...
            pba->Omega0_lambda * pow(pba->H0,2)
            * exp(-4.0*theta_SCF(pba, a)) // here define CSF(tau)
            ;
            rho_tot += pvecback[pba->index_bg_rho_lambda];
            p_tot -= pvecback[pba->index_bg_rho_lambda];
        }
...
int background_init(
    ...
    if (pba->background_verbose > 0) {
        ...
        printf("Computing background (CLASS_CSF)\n");
    }
...
//B CLASS_CSF
    if (pba->background_verbose > 0) {
        printf(" -> initial condition at a = %e\n",a);
        printf("    corresponding to proper time = %e Mpc\n",
                pvecback_integration[pba->index_bi_time]);
        printf("    corresponding to conformal time = %e Mpc\n",
                pvecback_integration[pba->index_bi_tau]);
        printf("    corresponding to Friedmann H = %e Mpc^-1\n",
                pvecback[pba->index_bg_H]);
        printf("    corresponding to rs = %e \n",
                pvecback_integration[pba->index_bi_rs]);
        printf("    corresponding to D = %e \n",
                pvecback_integration[pba->index_bi_D]);
        printf("    corresponding to D_prime = %e \n",
                pvecback_integration[pba->index_bi_D_prime]);
        printf("    corresponding to loga_ini = %e \n",
                *loga_ini);
    }
//E
...
//B EMT::
//    printf("has_EMT %d\n",pba->has_EMT);
//    printf("bg, bi, sizes %d %d\n",
//                             pba->bg_size,pba->bi_size);
//E
  ...
  double *dataptr, *pvecback;
    ...
    if (pba->background_verbose > 0) {
        printf("background_output_data: has_cdm and has_lambda %d %d\n",pba->has_cdm, pba->has_lambda);
#ifdef CLASS_CSF
        printf("background_output_data: has_CSF %d\n",
               pba->has_CSF);
#endif
        printf("background_output_data: bg, bi, sizes %d %d\n",
               pba->bg_size,pba->bi_size);
    }
...
//B CLASS_CSF
double theta_SCF(struct background *pba, double a)
{
    return pba->QCSF*tanh(3.5*(1.0-a));
}
//E
